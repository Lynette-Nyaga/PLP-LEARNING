...
const cashRegister = {
    ...
    calculateTotalPrice: function() {
        let totalPrice = this.shoppingCart.reduce((total, currentItem) => {
            return total + this.products[currentItem];
        }, 0);
    ...
    pay: function(paymentAmount) {
        ...
        if (paymentAmount < totalPrice) {
            ...
            if (paymentAmount > totalPrice) {
                console.log(`Your change is $${(paymentAmount - totalPrice).toFixed(2)}`);
            }
        }
    }
};
...
cashRegister.pay(1000); // Adjust payment amount for testing
